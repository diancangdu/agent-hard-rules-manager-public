...
- **`RULE_023` 本地模型用时加载、不用时关闭**（notebook 层，规则总数 14 → **15**）。
  本机部署的模型（含本地推理服务）一律**用时加载、不用时关闭**：不得为了「以后可能用到」而常驻，
  也不得在任务结束后把服务留在后台占显存/内存。
  - **判据是客观的**：任务完成后对应端口应释放（`Get-NetTCPConnection`）、显存应回落（`nvidia-smi`）。
  - 明确要求**把「按需加载 + 空闲卸载」做进工具本身**，而不是靠人记得关 ——
    例如插件在调用时拉起服务、空闲超时后杀掉**自己拉起的**进程树。
  - 只授权关闭自己拉起的实例；别人启动的服务只报告、不动手。
  - 归入 notebook 层而非 hook 层：这是流程纪律 + 跨调用状态，单次工具调用层面无法机械判定。

### 验证

- `check_situation` 命中 RULE_023，`total_rules_in_notebook: 15`
- 全量硬闸回归 **30/30**（RULE_023 为 notebook 层，不影响 hook 规则集），
  且真实排障日志未被触碰
